...
Server::Server() : cb(false), id(0), loc_id(""), ip(""), port(""), host(""), root(""), o_root(""), index(), error_page(), client_max_body_size("")
...
Server::Server(const Server &src) : cb(src.cb), id(src.id), loc_id(src.loc_id), ip(src.ip), port(src.port), host(src.host), root(src.root)
